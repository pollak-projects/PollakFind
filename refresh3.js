  ...
  function setupMobile() {
    if (!document.querySelector(".mobile-floor-nav")) {
      ...
      mobileNav.innerHTML = `
        ...
        <span id="currentFloorLabel">Emelet: ${currentFloor === 0 ? "Földszint" : currentFloor + ". Emelet"}</span>
        <button id="nextFloor">&rarr;</button>
      `;
      document.body.appendChild(mobileNav);
      document.getElementById("prevFloor").addEventListener("click", function() {
      ...
      document.getElementById("nextFloor").addEventListener("click", function() {
        if (floors[currentFloor + 1] !== undefined) {
          currentFloor = currentFloor + 1;
          switchFloor(currentFloor);
          updateFloorLabel();
      ...
      function updateFloorLabel() {
        document.getElementById("currentFloorLabel").innerText = `Emelet: ${currentFloor === 0 ? "Földszint" : currentFloor + ". Emelet"}`;
      }
    }
  }
...
// Pathfinding és segédfüggvények
